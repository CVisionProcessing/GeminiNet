...
class Vgg16:
    def __init__(self, vgg16_npy_path=None):
        if vgg16_npy_path is None:
            path = sys.modules[self.__class__.__module__].__file__ #sys.modules是全局字典，里面存储了python启动时加载了哪些模块,self.
            # print path
            path = os.path.abspath(os.path.join(path, os.pardir))#返回模块文件的上层目录
            # print path
