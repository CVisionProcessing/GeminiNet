...
class Model:
    def __init__(self):
        ...
        self.label_noise = tf.placeholder(tf.float32, [1,label_size,label_size,1])
    ...
    def build_model(self):
        ...
        with tf.variable_scope('fusion') as scope:
            ...
            att_1= self.squeeze_excitation_layer(conv1_dilation, out_dim=512, ratio=4, layer_name='squeeze_layer_5')
            att_3= self.squeeze_excitation_layer(conv2_dilation, out_dim=512, ratio=4, layer_name='squeeze_layer_4')
            att_5= self.squeeze_excitation_layer(conv3_dilation, out_dim=512, ratio=4, layer_name='squeeze_layer_3')
            att_7= self.squeeze_excitation_layer(conv4_dilation, out_dim=512, ratio=4, layer_name='squeeze_layer_2')
            att_9= self.squeeze_excitation_layer(conv5_dilation, out_dim=512, ratio=4, layer_name='squeeze_layer_1')
            ...
            all_fea = tf.concat([prev1, prev2, prev3, prev4],axis=3)
            all_fea = self.Conv_2d(all_fea, [1, 1, 320, 320], 0.01, name='a1l_fea')
            fg_fea=all_fea*self.label_noise
            bg_fea=all_fea*(1-self.label_noise)
            c1=tf.reduce_sum(tf.reduce_sum(fg_fea,axis=1,keep_dims=True),axis=2,keep_dims=True)/tf.reduce_sum(self.label_noise)
            c2=tf.reduce_sum(tf.reduce_sum(bg_fea,axis=1,keep_dims=True),axis=2,keep_dims=True)/tf.reduce_sum(1-self.label_noise)

            dist1=tf.sqrt(tf.reduce_sum((all_fea-c1)**2,axis=3,keep_dims=True))
            dist2=tf.sqrt(tf.reduce_sum((all_fea-c2)**2,axis=3,keep_dims=True))
            dist = tf.concat([-dist1,-dist2],axis=3)

        self.Score = tf.reshape(dist, [-1, 2])
